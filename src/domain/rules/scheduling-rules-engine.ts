...
export class SchedulingRulesEngine {
    /** 
     *  checks if voluntter can sign up for shift 
     * this validates alll scheduling at once
     * returns { isAllowerd: boolean, errors: string[] }
     */
    validateSignup(
        volunteer: Volunteer,
        shift: Shift,
    ...
    private checkMaxKakadsPerMonth( 
        ...
        if (shift.type !== ShiftType.KAKAD) {
          return { isAllowed: true, errors: [] }; // This rule only applies to Kakad shifts
        }
        ...
        if (kakadCount >= MAX_KAKAD) {
          return {
            isAllowed: false,
            errors: [`Cannot sign up: You already have ${kakadCount} 2 Kakad shifts this month (max is ${MAX_KAKAD})`],
          };
        }
      ...
      /**
       * Rule 3: Max 4 total shifts per volunteer per month
       */
    private checkMaxTotalPerMonth(
        volunteer: Volunteer, 
        shift: Shift, 
        existingSignups: Signup[]): { isAllowed: boolean, errors: string[] } {
        const shiftMonth = shift.getMonthYear();
        ...
        if (totalCount >= MAX_TOTAL) {
            return { 
              isAllowed: false, 
              errors: [`Cannot sign up: You already have ${totalCount} 4 shifts this month (max is ${MAX_TOTAL})`]
            };
        }
    ...
    /**
     * Rule 4: Max 2 Thurs shifts per volunteer per month
     */
    private checkMaxThursdayPerMonth(
        ...
        if (thursdayCount >= MAX_THURSDAY) {
            return {
                isAllowed: false, 
                errors: [`Cannot sign up: You already have ${thursdayCount} 2 Thursday shifts this month (max is ${MAX_THURSDAY})`],
            };
        }
    ...
    /**
     * Rule 5: Max 1 Robe shift per volunteer per day
     */
    ...
    if (shift.type !== ShiftType.ROBE) {
      return { isAllowed: true, errors: [] }; // This rule only applies to Robe shifts
    }
